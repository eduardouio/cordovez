...
CREATE TABLE IF NOT EXISTS `appImport`.`proveedor` (
  ...
  `categoria` VARCHAR(60) NOT NULL COMMENT 'TIPO DE SERVICIO PRESTADO PRO EL PROVEEDOR',
...
CREATE TABLE IF NOT EXISTS `appImport`.`producto` (
  ...
  `custodia_doble` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Define el costo de la custodia cuando esta es doble',
  `date_create` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `last_update` DATETIME NOT NULL,
  `id_user` SMALLINT NOT NULL COMMENT 'NOMBRE APP USER QUE GUARDA EL REGISTRO',
